...
import com.app.spark.image_cropper.CropImageView.CropShape
import com.app.spark.image_cropper.CropImageView.Guidelines
import java.util.*
import kotlin.math.*
...
@JvmOverloads constructor(
    ...
    private var mCropWindowChangeListener: CropWindowChangeListener? =
        null
    ...
    private val mBoundsPoints: FloatArray? = FloatArray(8)

    /** The bounding box around the Bitmap that we are cropping.  */
    private val mCalcBounds: RectF? = RectF()
     ...
     * mTargetAspectRatio).
     */
    private var mFixAspectRatio = true

    /** save the current aspect ratio of the image  */
    private var mAspectRatioX = 4

    /** save the current aspect ratio of the image  */
    private var mAspectRatioY = 4
     ...
     * application.
     */
    private fun setGuidelines(guidelines: Guidelines?) {
    ...
    private fun setAspectRatioX(aspectRatioX: Int) {
     ...
     */
    private fun setAspectRatioY(aspectRatioY: Int) {
     ...
     * box edge. (default: 3)
     */
    private fun setSnapRadius(snapRadius: Float) {
        mSnapRadius = snapRadius
    }

    /** Set multi touch functionality to enabled/disabled.  */
    private fun setMultiTouchEnabled(multiTouchEnabled: Boolean): Boolean {
    ...
    fun getInitialCropWindowRect(): Rect? {
    ...
    fun setInitialAttributeValues(options: CropImageOptions?) {
       ...
       setAspectRatioX(4)
        ...
        mBackgroundPaint =
            getNewPaint(options.backgroundColor)
    }
    ...
    private fun initCropWindow() {
        ...
        val rightLimit =
            min(BitmapUtils.getRectRight(mBoundsPoints), width.toFloat())
        val bottomLimit =
            min(BitmapUtils.getRectBottom(mBoundsPoints), height.toFloat())
        ...
        val horizontalPadding =
            mInitialCropWindowPaddingRatio * (rightLimit - leftLimit)
        val verticalPadding =
            mInitialCropWindowPaddingRatio * (bottomLimit - topLimit)
        if (mInitialCropWindowRect!!.width() > 0 && mInitialCropWindowRect.height() > 0) {
            ...
            val bitmapAspectRatio =
                (rightLimit - leftLimit) / (bottomLimit - topLimit)
    ...
    private fun fixCropWindowRectByRules(rect: RectF?) {
        ...
        if (mCalcBounds!!.width() > 0 && mCalcBounds.height() > 0) {
            ...
            val rightLimit =
                min(mCalcBounds.right, width.toFloat())
            val bottomLimit =
                min(mCalcBounds.bottom, height.toFloat())
        ...
        if (mFixAspectRatio && abs(rect.width() - rect.height() * mTargetAspectRatio) > 0.1) {
            if (rect.width() > rect.height() * mTargetAspectRatio) {
                ...
                val adj =
                    abs(rect.width() / mTargetAspectRatio - rect.height()) / 2
                rect.top += adj
                rect.bottom -= adj
            }
        }
    ...
    private fun drawCorners(canvas: Canvas?) {
        if (mBorderCornerPaint != null) {
            val lineWidth: Float =
                if (mBorderPaint != null) mBorderPaint!!.strokeWidth else 0f
            ...
            val w: Float = (cornerWidth.div(2)
                .plus(if (mCropShape == CropShape.RECTANGLE) mBorderCornerOffset else 0f))
            val rect = mCropWindowHandler!!.getRect()
            rect!!.inset(w, w)
