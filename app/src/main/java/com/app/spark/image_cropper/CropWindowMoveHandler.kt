...
internal class CropWindowMoveHandler(
    ...
    // --Commented out by Inspection START (9/5/20 4:39 PM):
    //  /** Matrix used for rectangle rotation handling */
    //  private static final Matrix MATRIX = new Matrix();
    // --Commented out by Inspection STOP (9/5/20 4:39 PM)
    /** Minimum width in pixels that the crop window can get.  */
    private val mMinCropWidth: Float = cropWindowHandler!!.getMinCropWidth()

    /** Minimum width in pixels that the crop window can get.  */
    private val mMinCropHeight: Float = cropWindowHandler?.getMinCropHeight() ?: 0f

    /** Maximum height in pixels that the crop window can get.  */
    private val mMaxCropWidth: Float = cropWindowHandler?.getMaxCropWidth() ?: 0f

    /** Maximum height in pixels that the crop window can get.  */
    private val mMaxCropHeight: Float = cropWindowHandler?.getMaxCropHeight() ?: 0f
     ...
     */
    private val mTouchOffset: PointF = PointF()
     ...
     * handle.
     */
    private fun calculateTouchOffset(
        rect: RectF,
        touchX: Float,
        touchY: Float
    ) {
    ...
    private fun adjustRight(
        ...
        if (aspectRatio > 0) {
            ...
            if (newHeight < mMinCropHeight) {
                newRight =
                    min(bounds.right, rect.left + mMinCropHeight * aspectRatio)
            ...
            if (newHeight > mMaxCropHeight) {
                newRight =
                    min(bounds.right, rect.left + mMaxCropHeight * aspectRatio)
                newHeight = (newRight - rect.left) / aspectRatio
            }
    ...
    private fun adjustBottom(
        ...
        if (aspectRatio > 0) {
            ...
            if (newWidth < mMinCropWidth) {
                newBottom =
                    min(bounds.bottom, rect.top + mMinCropWidth / aspectRatio)
            ...
            if (newWidth > mMaxCropWidth) {
                newBottom =
                    min(bounds.bottom, rect.top + mMaxCropWidth / aspectRatio)
                newWidth = (newBottom - rect.top) * aspectRatio
            }
     ...
     */
    private fun adjustLeftRightByAspectRatio(
        rect: RectF?,
        bounds: RectF?,
        aspectRatio: Float
    ) {
     ...
     */
    private fun adjustTopBottomByAspectRatio(
        rect: RectF?,
        bounds: RectF?,
        aspectRatio: Float
    ) {
