...
import com.app.spark.image_cropper.CropImageView.RequestSizeOptions
import java.io.*
...
internal object BitmapUtils {
    ...
    var POINTS: FloatArray? = FloatArray(6)

    /** Reusable point for general internal usage  */
    var POINTS2: FloatArray? = FloatArray(6)
    ...
    var mStateBitmap: Pair<String?, WeakReference<Bitmap?>?>? =
        null
    ...
    fun rotateBitmapByExif(
        ...
        try {
            ...
            if (`is` != null) {
                ei = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    ExifInterface(`is`)
                } else ExifInterface(uri?.path!!)
                `is`.close()
            }
        } catch (ignored: Exception) {
        }
        return ei?.let { rotateBitmapByExif(bitmap, it) }
            ?: RotateBitmapResult(
                bitmap,
                0
            )
    ...
    fun getRectRight(points: FloatArray?): Float {
        return points!![0].coerceAtLeast(points[2]).coerceAtLeast(points[4])
    ...
    fun getRectFromPoints(
        ...
        val left =
            max(0f, getRectLeft(points)).roundToLong()
        val top = max(0f, getRectTop(points)).roundToLong()
        val right = min(
            imageWidth.toFloat(),
            getRectRight(points)
        ).roundToLong()
     ...
     */
    private fun fixRectForAspectRatio(
        rect: Rect,
        aspectRatioX: Int,
        aspectRatioY: Int
    ) {
    ...
    fun writeTempStateStoreBitmap(
        ...
        return try {
            ...
            } else if (File(uri1.path!!).exists()) {
                needSave = false
            }
            if (needSave) {
                writeBitmapToUri(context, bitmap, uri1, CompressFormat.JPEG, 95)
            }
            uri1
        } catch (e: Exception) {
            Log.w(
    ...
    private fun cropBitmap(
        ...
        try {
            ...
            val fullBitmap =
                decodeImage(context!!.contentResolver, loadedImageUri, options)
            if (fullBitmap != null) {
                try {
    ...
    @Throws(FileNotFoundException::class)
    private fun decodeImageForOption(
        ...
        if (degreesRotated % 90 != 0) {
            ...
            while (i < points!!.size) {
                if (points[i] >= compareTo - 1 && points[i] <= compareTo + 1) {
                    adjLeft =
                        abs(sin(rads) * (rect.bottom - points[i + 1])).toInt()
                    adjTop =
                        abs(cos(rads) * (points[i + 1] - rect.top)).toInt()
                    width =
                        abs((points[i + 1] - rect.top) / sin(rads)).toInt()
                    height = abs(
                        (rect.bottom - points[i + 1]) / cos(rads)
                    ).toInt()
                    break
                }
                i += 2
            ...
            val bitmapTmp = bitmap1
            bitmap1 = Bitmap.createBitmap(bitmap1!!, rect.left, rect.top, rect.width(), rect.height())
            if (bitmapTmp != bitmap1) {
                bitmapTmp?.recycle()
            }
        }
        return bitmap1
    ...
    private fun getMaxTextureSize(): Int {
        ...
        val imageMaxBitmapDeduction = 2048
        return try {
            // Get EGL Display
            ...
            max(maximumTextureSize, imageMaxBitmapDeduction)
        } catch (e: Exception) {
            imageMaxBitmapDeduction
        }
    }
