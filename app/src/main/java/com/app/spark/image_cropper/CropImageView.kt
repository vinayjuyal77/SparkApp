...
import com.app.spark.image_cropper.BitmapCroppingWorkerTask
...
class CropImageView @JvmOverloads constructor(
    ...
    private var mImagePoints: FloatArray? = FloatArray(8)

    /** Rectangle used in image matrix transformation for scale calculation (reusing rect instance)  */
    private var mScaleImagePoints: FloatArray? = FloatArray(8)
     ...
     */
    private var mSaveBitmapToInstanceState = false
    ...
    private var mBitmapLoadingWorkerTask: WeakReference<BitmapLoadingWorkerTask?>? =
        null

    /** Task used to crop bitmap async from UI thread  */
    private var mBitmapCroppingWorkerTask: WeakReference<BitmapCroppingWorkerTask?>? =
        null
    ...
    fun getCropPoints(): FloatArray? {
        ...
        val cropWindowRect: RectF = mCropOverlayView?.getCropWindowRect()!!
    ...
    fun setImageUriAsync(uri: Uri?) {
        if (uri != null) {
            (if (mBitmapLoadingWorkerTask != null) mBitmapLoadingWorkerTask!!.get() else null)?.cancel(true)
            ...
            mCropOverlayView?.setInitialCropWindowRect(null)
            mBitmapLoadingWorkerTask =
                WeakReference<BitmapLoadingWorkerTask?>(
                    BitmapLoadingWorkerTask(
                        this,
                        uri
                    )
                )
            mBitmapLoadingWorkerTask!!.get()
    ...
    fun rotateImage(degrees: Int) {
        ...
        if (mBitmap != null) {
            ...
            degrees1 = if (degrees1 < 0) {
                degrees1 % 360 + 360
            } else {
                degrees1 % 360
            }
            val flipAxes = (!(mCropOverlayView?.isFixAspectRatio()!!)
                    && (degrees1 in 46..134 || degrees1 in 216..304))
            BitmapUtils.RECT?.set(mCropOverlayView.getCropWindowRect()!!)
            var halfWidth =
                (if (flipAxes) BitmapUtils.RECT?.height() else BitmapUtils.RECT?.width())?.div(2f)
            var halfHeight =
                (if (flipAxes) BitmapUtils.RECT?.width() else BitmapUtils.RECT?.height())?.div(2f)
            if (flipAxes) {
            ...
            mDegreesRotated = (mDegreesRotated + degrees1) % 360
            ...
            halfWidth = halfWidth!! * change.toFloat()
            halfHeight = halfHeight!! * change.toFloat()
            ...
            applyImageMatrix(width.toFloat(), height.toFloat(), center = true, animate = false)
            handleCropWindowChanged(false, animate = false)
    ...
    private fun startCropWorkerTask(
        ...
        if (bitmap != null) {
            ...
            width = if (options != RequestSizeOptions.NONE) width else 0
            height = if (options != RequestSizeOptions.NONE) height else 0
            val orgWidth = bitmap.width * mLoadedSampleSize
            val orgHeight = bitmap.height * mLoadedSampleSize
            mBitmapCroppingWorkerTask = if (mLoadedImageUri != null
    ...
    public override fun onSaveInstanceState(): Parcelable? {
        ...
        if (mBitmapLoadingWorkerTask != null) {
            val task: BitmapLoadingWorkerTask? = mBitmapLoadingWorkerTask?.get()
            if (task != null) {
    ...
    public override fun onRestoreInstanceState(state: Parcelable?) {
        if (state is Bundle) {
            ...
            if (mBitmapLoadingWorkerTask == null && mLoadedImageUri == null && mBitmap == null && mImageResource == 0
                ...
                var uri =
                    bundle?.getParcelable<Uri?>("LOADED_IMAGE_URI")
                if (uri != null) {
                ...
                val initialCropRect =
                    bundle.getParcelable<Rect?>("INITIAL_CROP_RECT")
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (mBitmap != null) {
            ...
            if (widthSize < mBitmap?.width!!) {
                viewToBitmapWidthRatio =
                    widthSize.toDouble() / mBitmap?.width!!.toDouble()
            }
            if (heightSize < mBitmap?.height!!) {
                viewToBitmapHeightRatio =
                    (heightSize/ mBitmap!!.height).toDouble()
            }
            ...
            val width =
                getOnMeasureSpec(widthMode, widthSize, desiredWidth)
            val height =
                getOnMeasureSpec(heightMode, heightSize, desiredHeight)
            mLayoutWidth = width
    ...
    override fun onLayout(
        ...
        if (mLayoutWidth > 0 && mLayoutHeight > 0) {
            ...
            if (mBitmap != null) {
                applyImageMatrix(r - l.toFloat(), b - t.toFloat(), center = true, animate = false)
     ...
     */
    private fun handleCropWindowChanged(
        inProgress: Boolean,
        animate: Boolean
    ) {
        val width = width
        ...
        if (mBitmap != null && width > 0 && height > 0) {
            val cropRect: RectF = mCropOverlayView?.getCropWindowRect()!!
            if (inProgress) {
    ...
    private fun applyImageMatrix(
        ...
        if (mBitmap != null && width > 0 && height > 0) {
            ...
            mImageMatrix?.postTranslate(
                (width - mBitmap!!.width).div(2), (height - mBitmap!!.height).div(2)
            )
            ...
            if (center) {
                ...
                mZoomOffsetX =
                    ((if (width > BitmapUtils.getRectWidth(mImagePoints)) 0f else (width / 2 - cropRect.centerX()).coerceAtMost(
                        ...
                        .coerceAtLeast(getWidth() - BitmapUtils.getRectRight(mImagePoints))).div(scaleX))

                mZoomOffsetY =(if (height > BitmapUtils.getRectHeight(mImagePoints)) 0f else (height / 2 - cropRect.centerY()).coerceAtMost(
                        -BitmapUtils.getRectTop(mImagePoints)
                    )
                        .coerceAtLeast(getHeight() - BitmapUtils.getRectBottom(mImagePoints))).div(scaleY)
            } else {
    ...
    private fun mapImagePointsByImageMatrix() {
        ...
        mScaleImagePoints = floatArrayOf(0f,0f,100f,0f,100f,100f,0f,100f)
    ...
    private fun setCropOverlayVisibility() {
        if (mCropOverlayView != null) {
            mCropOverlayView.visibility =
                if (mShowCropOverlay && mBitmap != null) View.VISIBLE else View.INVISIBLE
        }
    }
        ...
        if (options == null) {
            ...
            if (attrs != null) {
                ...
                try {
                    ...
                    mSaveBitmapToInstanceState = ta.getBoolean(
