...
internal class BitmapLoadingWorkerTask(
    ...
    override fun doInBackground(vararg params: Void?): Result? {
        return try {
            if (!isCancelled) {
                val decodeResult =
                    BitmapUtils.decodeSampledBitmap(mContext?.get(), mUri, mWidth, mHeight)
                if (!isCancelled) {
                    val rotateResult =
                        BitmapUtils.rotateBitmapByExif(decodeResult?.bitmap, mContext?.get(), mUri)
                    return Result(
                        mUri,
                        rotateResult?.bitmap,
    ...
    init {
        ...
        val densityAdj: Double =
            (if (metrics?.density!! > 1) {
                1.toDouble() / metrics.density
            } else 1.toDouble())
