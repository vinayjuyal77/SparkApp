...
import com.app.spark.activity.story.utils.PausableScaleAnimation
...
class PausableProgressBar @JvmOverloads constructor(
    ...
    fun setMinWithoutCallback() {
        maxProgressView!!.setBackgroundResource(R.color.gray_text_color)
    ...
    fun setMaxWithoutCallback() {
        maxProgressView!!.setBackgroundResource(R.color.green1)
        maxProgressView!!.visibility = View.VISIBLE
        if (animation != null) {
            animation!!.setAnimationListener(null)
            animation!!.cancel()
