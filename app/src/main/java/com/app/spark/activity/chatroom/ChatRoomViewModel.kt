...
class ChatRoomViewModel(application: Application) : AndroidViewModel(application),
    ...
    var isUserOffline = MutableLiveData<Boolean>()
    ...
    fun setInitialData(userId: String?, otherUserId: String?) {
        this.userId = userId
    ...
    var onReceiveChatList = Emitter.Listener { args ->
        ...
        if (response.result.onlineStatus.toLowerCase() == "offline") {
            isUserOffline.postValue(true)
        } else isUserOffline.postValue(false)
    ...
    private fun getChatListRequest() {
        ...
        try {
            ...
            chatListRequestJson.put(ServerConstant.CHAT_TYPE, "normal")
        } catch (e: JSONException) {
            e.printStackTrace()
        }
    ...
    fun sendMessage(
        ...
        try {
            ...
            sendMessageJson.put(ServerConstant.CHAT_TYPE, "normal")
            sendMessageJson.put(
    ...
    internal suspend fun loadImagesfromSDCard(selection: String, list: MutableList<MediaModel>) {
        ...
        if (imagecursor != null) {
            while (imagecursor.moveToNext() && list.size < 11) {
                val imageId: Long =
    ...
    fun isUpdateEngageApi( userId: Int) {
        if (isNetworkAvailable(getApplication())) {
            ...
            Coroutines.main {
                try {
                    val result = ApiInterface(getApplication()).isUpdateEngageApi(map)
                    Log.d("TAG", "isUpdateEngageApi: "+result.body()!!.statusCode)
                    if (result.isSuccessful) {
                        Log.d("isUpdateEngageApi", "isSuccessful")
                    } else {
                        errRes.postValue(R.string.something_went_wrong)
                    }
                } catch (ex: Exception) {
